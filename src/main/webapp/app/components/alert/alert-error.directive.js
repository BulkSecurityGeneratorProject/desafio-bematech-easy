(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('hoteisApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-hoteisApp-error');
                var entityKey = httpResponse.headers('X-hoteisApp-params');
                if (errorHeader) {
